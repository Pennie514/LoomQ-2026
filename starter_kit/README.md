...
## 🏆 本队参赛作品（SheNicest 2026 量子赛道）

参赛代码已完整实现 L1（三后端统一转译+模拟器）/ L2（智能体）/ L3（混合编译），
并附带真机接入、量子 RISC-V 扩展与 Web 交互界面。快速入口：
...
```bash
# 一键自测（L1/L3 无需模型服务）
python3 evaluator.py --level all --target spinq,originq,braket
# 量子 RISC-V 扩展端到端测试（Bonus）
